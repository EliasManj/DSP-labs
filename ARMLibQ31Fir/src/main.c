...
//				(float32_t *) &TempIn, (uint32_t) BUFFER_SIZE);
		...
		for (i = 0; i < BUFFER_SIZE; i++) {
			DACMem[i + (CurrMem << BUFFERAddreLeng)] = ((uint16_t) (Temp[i]
					+ 2048));
		}
...
void ADC3_CH12_DMA_Config(void) {
	...
	DMA_DoubleBufferModeConfig(DMA2_Stream0, (uint32_t) (&ADCMem[BUFFER_SIZE]),
			DMA_Memory_0);
...
void DAC_Ch2_Config(void) {
	...
	DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single;
	DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;
	DMA_DoubleBufferModeConfig(DMA1_Stream6, (uint32_t) (&DACMem[BUFFER_SIZE]),
			DMA_Memory_0);
