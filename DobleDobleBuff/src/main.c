...
volatile uint16_t buff0[BUFFER_SIZE]; //Array for ADC samples
volatile uint16_t buff1[BUFFER_SIZE]; //Array for ADC samples
uint16_t Sine12bit[32] = { 2047, 2447, 2831, 3185, 3498, 3750, 3939, 4056,
...
void DAC_Ch2_SineWaveConfig(void) {
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)(&buff1[0]);
	...
	/* Enable DMA1_Stream5 */
	DMA_DoubleBufferModeConfig(DMA1_Stream6, (uint32_t)(&buff0[0]),0x00);
...
void ADC3_CH12_DMA_Config(void) {
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t) (&buff0[0]);
	...
	DMA_DoubleBufferModeConfig(DMA2_Stream0, (uint32_t)(&buff1[0]),0x00);
